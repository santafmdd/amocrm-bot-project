...
def build_call_review_case_messages(
    *,
    ...
    system_prompt = (
        ...
        "Запрещено сравнивать CRM-этап с фактом разговора и писать формулировки вида "
        "\"в CRM этап..., а по факту...\" или \"CRM не отражает реальность звонка\". "
    ...
    user_prompt = (
        ...
        "Правила:\n"
        "0) primary_case_type определяй по содержанию транскрипта и факту разговора. "
        "CRM статус/стадия можно использовать только как дополнительный сигнал, но не как основной источник.\n"
...
def build_call_review_free_form_messages(
    *,
    ...
    system_prompt = (
        ...
        "Запрещено писать про несоответствие CRM-этапа факту звонка. "
        f"{style_line}"
    )
    user_prompt = (
