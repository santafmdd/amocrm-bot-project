...
Схема записи:
- writer использует фиксированный ordered schema contract `call_review_v3_row_schema.py` (индексная схема);
- заголовки листа читаются только для проверки ширины и dropdown-нормализации, а не как source of truth для маппинга;
- это устраняет коллизии при дублирующихся названиях колонок (`Здоровается`, `Комментарий по этапу`);
...
- суббота/воскресенье всегда относятся к понедельничному bucket;
