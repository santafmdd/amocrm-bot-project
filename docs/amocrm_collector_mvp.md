...
## 204 для notes/tasks
Если amoCRM возвращает для `notes`/`tasks` ответ `HTTP 204` с пустым body (даже при `content-type: text/html; charset=UTF-8`), collector трактует это как нормальный пустой результат:
- `items=[]`
- без warning
- без error

Warning по section создаётся только для реально подозрительных случаев: непустой non-JSON body, неуспешный HTTP статус, либо иная ошибка запроса/парсинга.

Команда `debug-deal-sections` в таком случае показывает `response_kind=empty_no_content`.
